...
class News {
    public draw(data: Readonly<IArticle[]>): void {
        const news: Readonly<IArticle[]> = data.length >= 10 ? data.filter((_item: IArticle, idx: number) => idx < 10) : data;

        const fragment: DocumentFragment = document.createDocumentFragment();
        const newsItemTemp: HTMLTemplateElement | null = document.querySelector('#newsItemTemp');

        news.forEach((item: IArticle, idx: number) => {
            const newsClone: DocumentFragment = (newsItemTemp as HTMLTemplateElement).content.cloneNode(
                true
            ) as DocumentFragment;
